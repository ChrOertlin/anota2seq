...
In this vignette, we will use simulated data provided with the package to illustrate how to perform each step of the analysis. These data originate from the study by Oertlin et al. \cite{Oertlin} which compared methods for analysis of translatomes quantified by RNAseq. Eight samples were simulated from 2 sample classes ("control" and "treatment"); both total mRNA (anota2seqDataT, raw RNAseq counts) and paired translated mRNA (anota2seqDataP, raw RNAseq counts) are provided together with a sample class vector (anota2seqPhenoVec).
...
countData <- as.matrix(cbind(anota2seqDataP[1:1000,],anota2seqDataT[1:1000,]))
...
\subsubsection{Important note about analysis of translational buffering}
An important note when considering changes in translational efficiency leading to buffering is that a buffered down mRNA will, in the statistical output of anota2seq, have a \emph{positive} log(effect) and inversely, a buffered up gene will have a \emph{negative} log(effect). This can be counterintuitive and requires additional explanation:\\
...
Translational buffering is defined as changes in total mRNA level that are not paralleled by changes in levels of translated mRNA. As such, performing analysis of buffering considers total mRNA as independent variable and translated mRNA as dependent variable (together with the sample class; as illustrated in Fig. \ref{fig:singleGeneRegressions_buffering}). Semantically, from a translational control perspective, a mRNA which is called "buffering down" is a mRNA for which the total mRNA logFC is positive and for which this change is buffered at the translational level maintaining translated mRNA constant. For this "buffering down" mRNA, in the above described model, the difference in intercepts will be positive. 
